...
/**
 * Class Waf_WC_VGC_Gateway
 */
class Waf_WC_Vgc_Gateway extends WC_Payment_Gateway {
	...
	public function __construct() {
		$this->id                 = 'waf_vgc';
		$this->method_title       = 'Very Good Collection';
        $this->order_button_text = __( 'Proceed to Very Good Collection', 'woocommerce' );
        $this->method_title      = __( 'Very Good Collection', 'woocommerce' );
		$this->method_description = sprintf( 'Receive money from anyone with Our Borderless Payment Collection Platform. Payout straight to your bank account. <a href="%1$s" target="_blank">Sign up</a> for a Very Good Collection account, and <a href="%2$s" target="_blank">get your API keys</a>.', 'https://verygoodcollection.com/', 'https://verygoodcollection.com/user/api' );
        $this->icon = WC_HTTPS::force_https_url( plugins_url( 'assets/images/powered_by_verygoodcollection.png', WAF_WC_VGC_MAIN_FILE ) );
		...
		add_action( 'woocommerce_api_waf_wc_vgc_gateway', array( $this, 'verify_vgc_transaction' ) );
		// Webhook listener/API hook.
		add_action( 'woocommerce_api_vgc_success', array( $this, 'process_success' ) );
        add_action( 'woocommerce_api_vgc_proceed', array( $this, 'vgc_proceed' ) );
	}

    /**
     * @param bool $string
     * @return array|string
     * Get currently supported currencies from verygoodcollection endpoint
     */
	...
	/**
	 * Check if VGC merchant details is filled
	 */
	public function admin_notices() {
		...
		if ( ! ( $this->public_key && $this->secret_key ) ) {
			echo '<div class="error"><p>' . sprintf( 'Please enter your Very Good Collection merchant details <a href="%s">here</a> to be able to use the Very Good Collection WooCommerce plugin.', admin_url( 'admin.php?page=wc-settings&tab=checkout&section=waf_vgc' ) ) . '</p></div>';
			return;
		}
	...
	/**
	 * Check if Very Good Collection gateway is enabled.
	 */
	...
	public function admin_options() {
		...
		<h3>Very Good Collection</h3>
        <h4>Our Supported Currencies: <?php echo $this->get_supported_currencies(true); ?></h4>
		<?php
	...
	public function init_form_fields() {
		...
		$this->form_fields = array(
			'enabled'         => array(
				'title'       => __( 'Enable/Disable', 'woo-vgc' ),
				'label'       => __( 'Enable Very Good Collection', 'woo-vgc' ),
				'type'        => 'checkbox',
				'description' => __( 'Enable Very Good Collection as a payment option on the checkout page.', 'woo-vgc' ),
				'default'     => 'no',
				'desc_tip'    => false,
			),
			'title'           => array(
				'title'       => __( 'Title', 'woo-vgc' ),
				'type'        => 'text',
				'description' => __( 'This controls the payment method title which the user sees during checkout.', 'woo-vgc' ),
				'desc_tip'    => false,
				'default'     => __( 'Very Good Collection', 'woo-vgc' ),
			),
			'description'     => array(
				'title'       => __( 'Description', 'woo-vgc' ),
				'type'        => 'textarea',
				'description' => __( 'This controls the payment method description which the user sees during checkout.', 'woo-vgc' ),
				'desc_tip'    => false,
				'default'     => __( 'Make payment using your debit, credit card & bank account', 'woo-vgc' ),
			),
            'invoice_prefix' => array(
                'title'       => __( 'Invoice Prefix', 'woo-vgc' ),
                'type'        => 'text',
                'description' => __( 'Please enter a prefix for your invoice numbers. If you use your Very Good Collection account for multiple stores ensure this prefix is unique as Very Good Collection will not allow orders with the same invoice number.', 'woo-vgc' ),
                'default'     => 'WC_',
                'desc_tip'    => false,
            ),
            'merchant_key' => array(
                'title'       => __( 'Merchant Key', 'woo-vgc' ),
                'type'        => 'text',
                'description' => __( 'Required: Enter your Merchant Key here. You can get your Public Key from <a href="https://verygoodcollection.com/user/merchant">here</a>', 'woo-vgc' ),
                'default'     => '',
                'desc_tip'    => false,
            ),
			'public_key' => array(
				'title'       => __( 'Public Key', 'woo-vgc' ),
				'type'        => 'text',
				'description' => __( 'Required: Enter your Public Key here. You can get your Public Key from <a href="https://verygoodcollection.com/user/api">here</a>', 'woo-vgc' ),
				'default'     => '',
				'desc_tip'    => false,
			),
			'secret_key' => array(
				'title'       => __( 'Secret Key', 'woo-vgc' ),
				'type'        => 'text',
				'description' => __( 'Required: Enter your Secret Key here. You can get your Secret Key from <a href="https://verygoodcollection.com/user/api">here</a>', 'woo-vgc' ),
				'default'     => '',
				'desc_tip'    => false,
			)
		);
    ...
    public function process_payment( $order_id ) {
        ...
        $merchant_key = urlencode($this->merchant_key);
        ...
        $callback_url = urlencode(WC()->api_request_url( 'Vgc_Success' ) . "?order_id=" . $order_id);
        ...
        $url = WC()->api_request_url( 'Vgc_Proceed' ) . "?merchant_key={$merchant_key}&public_key={$public_key}&callback_url={$callback_url}&return_url={$callback_url}&tx_ref={$tx_ref}&amount={$amount}&email={$email}&first_name={$first_name}&last_name={$last_name}&title={$title}&currency={$currency_code}";
        //Return to Vgc Proceed page for the next step
        return array(
            'result' => 'success',
            'redirect' => $url
        );
    ...
    /**
     * API page to handle the callback data from Very Good Collection
     */
    public function process_success(){
        if($_GET['order_id']){
            $order_id = intval(sanitize_text_field($_GET['order_id']));
            $wc_order = wc_get_order($order_id);
            $vgc_request = wp_remote_get("https://verygoodcollection.com/api/verify-payment/{$this->invoice_prefix}{$order_id}/{$this->secret_key}");
            if ( ! is_wp_error( $vgc_request ) && 200 == wp_remote_retrieve_response_code( $vgc_request ) ) {
                $vgc_order = json_decode( wp_remote_retrieve_body( $vgc_request ) );
                $status = $vgc_order->status;
                if($status === "success"){
                    $order_total = $wc_order->get_total();
                    $amount_paid = $vgc_order->data->amount;
                    $reference_id = $vgc_order->data->reference;
                    ...
                    if($amount_paid < $order_total){
                        ...
                        $wc_order->add_order_note( sprintf( 'Payment via Very Good Collection successful (<strong>Reference ID:</strong> %s)', $reference_id ) );
                    }
                    ...
                    $wc_order->update_status( 'failed', 'Payment was declined by Very Good Collection.' );
                    wc_add_notice( 'Payment was declined by Very Good Collection.', 'error' );
                    wp_redirect( wc_get_page_permalink( 'checkout' ) );
                    die();
                }
            }
    ...
    /**
     * API page to redirect user to Very Good Collection
     */
    public function vgc_proceed(){
        $invalid = 0;
        if(!empty($_GET['merchant_key']) && !empty($_GET['public_key']) && wp_http_validate_url($_GET['callback_url'])){
            $merchant_key = sanitize_text_field($_GET['merchant_key']);
